<template>
  <UForm :state="state" :schema="schema" @submit.prevent="saveProfile">
    ...
    <UFormGroup class="mb-4" label="Email" name="email"
                help="You will receive a confirmation email on both the new and old addresses if you modify the email address.">
      <UInput v-model="state.email"></UInput>
    </UFormGroup>
    <UButton type="submit" color="black" variant="solid" label="save" :loading="pending" :disabled="pending"></UButton>
  </UForm>
</template>
<script setup>
import {z} from 'zod';
